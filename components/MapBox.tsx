...
export default function MapBox({
  ...
  }, [activeLayer, heatData, zoom, timer, realSolarBuildings, floodGridData]);

  // Temporary debug: add Google Maps markers for each solar building when solar layer active
  useEffect(() => {
    ...
    console.log("DEBUG: realSolarBuildings array:", realSolarBuildings);

    if (activeLayer === "solar" && Array.isArray(realSolarBuildings) && realSolarBuildings.length > 0) {
      realSolarBuildings.forEach((b: any, idx: number) => {
        try {
          console.log(`DEBUG: solar point ${idx}: lat=${b.lat}, lng=${b.lng}, id=${b.id}`);
          const marker = new google.maps.Marker({
            position: { lat: b.lat, lng: b.lng },
            map: mapInstanceRef.current as google.maps.Map,
            title: b.id || `solar-${idx}`,
            zIndex: 9999,
            optimized: false,
            icon: {
          ...
          // ignore early init issues
        }
      });
      console.log(`DEBUG: placed ${markersRef.current.length} google markers for solar points`);
    }
  ...
  }, [activeLayer, realSolarBuildings]);
